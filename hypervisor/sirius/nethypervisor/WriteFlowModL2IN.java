...
public class WriteFlowModL2IN extends Thread{
	...
	public WriteFlowModL2IN(IOFSwitch swSrc, IOFSwitch swDst, OFFlowModCommand command, OFBufferId bufferId,
	...
	public void run(){
		...
		writeFlowModL2IN(swSrc, swDst, command, bufferId, match, outPort, outPortDst, direct, server, virtualSwitch, 
	...
	private synchronized void writeFlowModL2IN(IOFSwitch swSrc, IOFSwitch swDst, OFFlowModCommand command, OFBufferId bufferId,
			Match match, OFPort outPort, OFPort outPortDst, boolean direct, Server3 server, VirtualSwitch virtualSrcSwitch, 
			TenantVirtualNetwork tenantVirtualNetwork,HashMap<Long,FlowEntry> cookieFlowEntryMap, 
			HashMap<Long,VirtualSwitch> cookieVirtualSwitchMap, HashMap<Long,VirtualNetwork> cookieVirtualNetworkMap, 
			EnvironmentOfTenants3 environmentOfTenants, EnvironmentOfServices3 environmentOfServers,
			HashMap<MacAddress, Host> macAddressHostMap, HashMap<Tenant3, Long> tenantTenantMacIdToVirtualMacsMap,
			HashMap<Tenant3, Long> tenantCurrentHostIdToVirtualMacsMap,	long tenantIdMacCount,
			long hostIdMacCount, long virtualMacAddressRaw, long start) {
		...
		if (direct){
			virtualSrcMac = Utils.virtualMacAddress(environmentOfServers.getHostLocationHostMap().get(new HostLocation(swSrc.getId(), match.get(MatchField.IN_PORT))), tenantVirtualNetwork, macAddressHostMap, 
					tenantTenantMacIdToVirtualMacsMap, tenantCurrentHostIdToVirtualMacsMap, tenantIdMacCount, hostIdMacCount, virtualMacAddressRaw, environmentOfTenants);
			virtualDstMac = Utils.virtualMacAddress(environmentOfServers.getHostLocationHostMap().get(new HostLocation(swDst.getId(), outPortDst)), tenantVirtualNetwork, macAddressHostMap, 
					tenantTenantMacIdToVirtualMacsMap, tenantCurrentHostIdToVirtualMacsMap, tenantIdMacCount, hostIdMacCount, virtualMacAddressRaw, environmentOfTenants);
			partialCookie = Utils.getPartialCookieWriteVirtualAndPhysicalFlows(match.get(MatchField.IN_PORT), server, virtualSrcSwitch, environmentOfTenants);

		} else{
			virtualDstMac= Utils.virtualMacAddress(environmentOfServers.getHostLocationHostMap().get(new HostLocation(swSrc.getId(), outPort)), tenantVirtualNetwork, macAddressHostMap, 
					tenantTenantMacIdToVirtualMacsMap, tenantCurrentHostIdToVirtualMacsMap, tenantIdMacCount, hostIdMacCount, virtualMacAddressRaw, environmentOfTenants);
			virtualSrcMac = Utils.virtualMacAddress(environmentOfServers.getHostLocationHostMap().get(new HostLocation(swDst.getId(), outPortDst)), tenantVirtualNetwork, macAddressHostMap, 
					tenantTenantMacIdToVirtualMacsMap, tenantCurrentHostIdToVirtualMacsMap, tenantIdMacCount, hostIdMacCount, virtualMacAddressRaw, environmentOfTenants);
			partialCookie = Utils.getPartialCookieWriteVirtualAndPhysicalFlows(outPort, server, virtualSrcSwitch, environmentOfTenants);
		}
		...
		if (partialCookie != 0L){
			...
			if(virtualSrcSwitch.getFlowTable().size() == 0){
				...
				virtualSrcSwitch.setFlowTable(flowTableArray);
				if(!Utils.hasFlowEntry(virtualSrcSwitch.getFlowTable().get(0).getFlowEntry(), match, al)){
					...
					virtualSrcSwitch.getFlowTable().get(0).addFlowEntry(flowEntry);
				}
				//TODO uncomment
//				System.out.println("VIRTUAL FLOW TABLE - "+virtualSrcSwitch.getName()+" - "+ virtualSrcSwitch.getFlowTable().get(0).toString());
			} else{
...
//				System.out.println("VIRTUAL FLOW TABLE - "+virtualSrcSwitch.getName()+" - "+ virtualSrcSwitch.getFlowTable().get(0).toString());
			...
			if(server.getOpenVSwitch().getFlowTable().size() == 0){
				...
				if(!Utils.hasFlowEntry(server.getOpenVSwitch().getFlowTable().get(0).getFlowEntry(), match, al)){
					...
					cookieVirtualSwitchMap.put(partialCookie + flowEntry.getFlowEntryId(), virtualSrcSwitch);
